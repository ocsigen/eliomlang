...
let intern () =
  ...
  let rec intern_rec () =
    let start = pos () in
    let code = read8u () in
    if code >= prefix_small_int then begin
      if code >= prefix_small_block then begin
        ...
        {desc = Int (code land 0x3F); start; fin = start + 1}
    end else begin
      if code >= prefix_small_string then begin
        ...
        match code with
          0x00 ->
            {desc = Int (read8s ()); start; fin = start + 2}
       | 0x01 ->
            {desc = Int (read16s ()); start; fin = start + 3}
        | 0x02 ->
            {desc = Int (read32s ()); start; fin = start + 5}
        | 0x03 ->
            assert false
        | 0x04 ->
            let ofs = read8u () in
        ...
        | 0x12 ->
            ...
            begin match ch with
              'j' ->
                let a = ref 0L in
                for i = 0 to 7 do
                  a := Int64.logor (Int64.shift_left !a 8)
                                   (Int64.of_int (read8u ()))
                done;
            ...
            | 'i' ->
                let a = ref 0l in
                for i = 0 to 3 do
                  a := Int32.logor (Int32.shift_left !a 8)
                                   (Int32.of_int (read8u ()))
                done;
            ...
            | 'n' ->
                ...
                for i = 0 to 7 do
                  a := Nativeint.logor (Nativeint.shift_left !a 8)
...
let rec print f v =
  if is_list v then begin
    ...
    match v.desc with
      Block (t, [||], n) ->
        Format.fprintf f "@[<1>{%d:}@]" t
    | Block (t, a, n) ->
        Format.fprintf f "@[<1>(%d){%d:" n t;
        Array.iter
          (fun v -> Format.fprintf f "@ %a" print v) a;
...
and print_list f v =
  match v.desc with
    Block (_, a, _) ->
      Format.fprintf f ";@ %a%a" print a.(0) print_list a.(1)
  | _ ->
      ()

let option f ppf v = match v.desc with
  | Block (0, [||], _) -> Format.fprintf ppf "None"
  | Block (0, [|a|], _ ) -> Format.fprintf ppf "Some(%a)" f a
  | _ -> assert false

let array f ppf v = match v.desc with
  | Block (0, v, _) ->
...
let _ =
  let v = intern () in
  match v.desc with
    Block (0, [|global_data; request_data|], _) ->
    let pr s f v =
      Format.printf "ZZ %s (%d)@." s (v.fin - v.start);
      Format.printf "@[%a@]@." f v
    in
    pr "global_data" (option @@ array print) global_data;
    pr "request_data" (array print) request_data;
  | _ -> assert false
